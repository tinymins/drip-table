...
>(props: SlotRenderProps<RecordType, ExtraOptions>) => {
  ...
  const renderColumnContent = (config: DripTableSlotElementSchema) => {
    ...
    if (config.type === 'slot') {
      ...
      const columnIndex = typeof props.columnKey === 'string' ? columnsUtils.findIndexRecursive(tableProps.schema.columns, c => c.key === props.columnKey) : -1;
      if (Slot) {
    ...
    if (config.type === 'display-column-selector') {
      const hidableColumns = columnsUtils.filterRecursive(tableProps.schema.columns, c => c.hidable);
