...
import * as columnsUtils from '@/utils/columns';
...
>(props: TableLayoutComponentProps): JSX.Element => {
  ...
  const hiddenColumnIndexes = React.useMemo(
    (): number[] => columnsUtils.flattenRecursive(tableProps.schema.columns)
  ...
  const rcTableInfo = React.useMemo(() => {
    const flattenColumns = columnsUtils.flattenRecursive(tableInfo.schema.columns);
  ...
  const rcTableColumnsRAW = React.useMemo(
    (): TableColumnsType<RcTableRecordType<RecordType>> => {
      ...
      const visibleColumns = columnsUtils.filterRecursive(
        tableProps.schema.columns,
        (_, i) => !hiddenColumnIndexes.includes(i),
      );
      // 树形结构转为拍平结构方便计算
      const flattenSchemaColumns: { schema: DripTableBaseColumnSchema; column: TableColumnType<RcTableRecordType<RecordType>> }[] = columnsUtils.flattenRecursive(visibleColumns)
  ...
  const flattenColumnsWidth = React.useMemo(
    () => {
      // 计算用户固定宽度列宽
      const userColumnsWidth = rcTableColumnsRAW.map(c => parseNumber(c.width, 0));
  ...
  const rcTableColumns = React.useMemo(
    (): TableColumnsType<RcTableRecordType<RecordType>> =>
      rcTableColumnsRAW.map((c, i) => ({ ...c, width: flattenColumnsWidth[i] })),
    ...
    body: tableInfo.schema.virtual
      ? (rawData, { scrollbarSize, onScroll }) => (
        <VariableSizeGrid
          ...
          itemData={{
            tableUUID,
            columns: rcTableColumns as TableColumnType<unknown>[], // TODO: TYPE
            columnsBaseSchema,
