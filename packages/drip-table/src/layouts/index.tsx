...
>(): JSX.Element => {
  ...
  React.useEffect(() => {
    setTableState(state => ({
      filters: Object.fromEntries(columnsUtils.flattenRecursive(tableProps.schema.columns).map(c => [c.dataIndex, c.defaultFilteredValue]).filter(([k, v]) => typeof k === 'string' && v)),
      displayColumnKeys: tableProps.displayColumnKeys || columnsUtils.flattenRecursive(tableProps.schema.columns).filter(c => c.hidable).map(c => c.key),
    }));
